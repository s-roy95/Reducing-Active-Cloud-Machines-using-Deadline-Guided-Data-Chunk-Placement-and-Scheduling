...
int main(){
    ...
    inputFile >> C;
    outfile<<endl;
    outfile<<"Test Case: "<<endl;
    outfile<<"Number of Jobs: "<<C<<endl;


    for (long long i = 0; i < C; i++) {
        long long x, y, d;
        inputFile >> d >> x;
        outfile<<"Deadline of Job"<<i+1<<" = "<<d<<", number of chunks in Job"<<i+1<<" = "<<x<<endl;
        chunks_sum += x;
        vector<long long> v;
        outfile<<"Chunks Array: ";
    ...
    inputFile >> S >> B;
    outfile<<"No. of VM's : "<<S<<", Maximum Capacity: "<<B<<endl<<endl;
    ...
    // if(ans<final_ans){
        for (int layer = 0; layer < ans; ++layer) {
            outfile << "Node " << layer+1 << ":" << endl;
            for (int row = 0; row < S; ++row) {
                outfile<<"VM "<<row+1<<":  ";
                for (int col = 0; col < dmax; ++col) {
                    outfile << ans_sch[layer][row][col] << " ";
                }
                outfile << endl;
            }
            outfile << endl;
        }
        outfile<<"The minimum number of nodes required is rand: "<<ans<<endl;
        outfile<<endl;
    // }
    // else{
        for (int layer = 0; layer < final_ans; ++layer) {
            ...
            for (int row = 0; row < S; ++row) {
                outfile<<"VM "<<row+1<<":  ";
                for (int col = 0; col < dmax; ++col) {
                    outfile << ans_sch1[layer][row][col] << " ";
                }
                outfile << endl;
            }
            outfile << endl;
        }
        outfile<<"The minimum number of nodes required is hue: "<<final_ans<<endl;
    // }
    outfile<<"-----------------------------------------------------------------------------------"<<endl;
    return 0;
}